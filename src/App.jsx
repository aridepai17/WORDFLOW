...
function App() {
  ...
  // const selectedPage = 2 // 0 is for welcome, 1 is for dashboard, 2 is for challenge
  ...
  useEffect(() => {
    ...
    if (!localStorage) {
      return;
    } // if we don't yet have access to the database, then exit the callback function

    if (localStorage.getItem("username")) {
      // if we find the item (so get item returns something), then we enter the if block
      setName(localStorage.getItem("username"));

      // we have a name, so we can move to the dashboard
      setSelectedPage(1);
    }
